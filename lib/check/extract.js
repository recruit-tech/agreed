...
module.exports.outgoingRequest = (req, opts) => {
  ...
  const query = req.query && qs.stringify(tryCompileQuery(req.query, req.values));
  if (query) {
    result.path += `?${query}`;
  }
  result.method = req.method;
...
function tryCompileQuery(query, values) {
  try {
    return JSON.parse(format(JSON.stringify(query), values));
  } catch (e) {
    return query;
  }
}
