...
module.exports = (agree, base) => {
  ...
  if (!agree.request.url) {
    throw new Error(`[agreed] request url is required ${JSON.stringify(agree.request)}`);
  }
  ...
  if (typeof agree.request.url !== 'string') {
    throw new Error(`[agreed] request url must be string ${JSON.stringify(agree.request)}`);
  }
  ...
  agree.request.pathToRegexp = pathToRegexp(agree.request.url, agree.request.pathToRegexpKeys); 
