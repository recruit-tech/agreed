...
class Server {
  ...
  useMiddleware(req, res, next) {
    ...
    extract.incomingRequest(req).then((req) => {
      ...
      const result = agreesWithResults
        .map((v, i) => [v, i])
        .sort((a, b) => {
          if (a[0].similarity !== b[0].similarity) {
            return b[0].similarity - a[0].similarity
          }
          return a[1] - b[1]
        })
        .map(([v]) => v)
      ...
      if (!result || !result.agree) {
        ...
        res.statusCode = 404;
        const { agree, result, similarity, error } = agreesWithResults.shift();
