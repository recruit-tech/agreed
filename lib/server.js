...
class Server {
  ...
  useMiddleware(req, res, next) {
    ...
    extract.incomingRequest(req).then((req) => {
      const agreesWithResults = stable(agrees.map((agree) => {
      ...
      if (this.options.strict && agreesWithResults.filter(({ result }) => result).length > 1) {
        res.header('Content-Type', 'application/json', 'charset=utf-8');
        res.statusCode = 500;
        res.statusMessage = "Ambiguous Request";
        res.end(JSON.stringify({
          candidates: agreesWithResults.filter(({ result }) => result).map(({ agree: {request, response} }) => {
